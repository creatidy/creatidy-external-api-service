...
class FiatCurrencyRates:
    ...
    def get_rate_pln(currency, transaction_date, only_working_days=False):
        ...
        if nbp_rate is None:  # No data in database
            ...
            if nbp_rate > 0:
                nbp.insert_rate(currency, date, nbp_rate)  # API OK - update DB
            else:  # No data from previous day, another trying with last working day
                before_holiday = HolidayCalculator.previous_working_day((transaction_date - datetime.timedelta(days=1)).date())
                nbp_rate = nbp.get_rate(currency, before_holiday)  # Check if rate is in DB?
                if nbp_rate <= 0:  # No data in DB
                    nbp_rate = nbp_api.get_rate(currency, before_holiday)  # Check NBP API
                    if nbp_rate > 0:
                        nbp.insert_rate(currency, date, nbp_rate)  # API OK - update DB with holiday date
                    else:
